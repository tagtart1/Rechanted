...
public class ModCreativeModeTabs {
    public static final DeferredRegister<CreativeModeTab> CREATIVE_MODE_TAB = DeferredRegister.create(Registries.CREATIVE_MODE_TAB, Rechanted.MOD_ID);

    public static final Supplier<CreativeModeTab> RECHANTED_TAB = CREATIVE_MODE_TAB.register("rechanted_tab",
            () -> CreativeModeTab.builder().icon(() -> new ItemStack(ModItems.CHANCE_GEM.get()))
                    .title(Component.translatable("creative.rechanted.title"))
                    .displayItems((itemDisplayParameters, output) -> {

                        output.accept(ModBlocks.RECHANTED_TABLE_BLOCK);
                        output.accept(ModBlocks.RECHANTED_TROPHY_BLOCK);
                            ...
                            for(EnchantmentPoolEntry enchantPoolEntry : bookRarityProperty.enchantmentPool) {
                                ...
                                enchantmentRegistry.get(key).ifPresent(enchantment -> {
                                    ...
                                    for(int i = 1 ; i <= actualMaxLevel ; i++) {
                                        ItemStack book = new ItemStack(ModItems.RECHANTED_BOOK.get());
